let scrollData=!0,scrollOffset=30,scrolltoTop=!1,scrollElement=null,uagbTOCCollapseListener=!0;UAGBTableOfContents={_getDocumentElement(){let e=document;var t=document.querySelectorAll('iframe[name="editor-canvas"]');return!t?.length||(t=t?.[0]?.contentWindow?.document||t?.[0]?.contentDocument)&&(e=t),e},updatePseudoElementWithSVG(d,t,o,l,s){const n=UAGBTableOfContents._getDocumentElement(),e=n.querySelector(l);if(e){var r=e.querySelector("li.uagb-toc__list:not(.uagb-toc__list--expandable)");if(r){r=window?.getComputedStyle(r).fontSize,r=r<"15px"?"15px":`calc(0.8 * ${r})`;let e=s;s.includes("var(")&&(a=s.match(/var\((.*?)\)/)[1],i=document.documentElement,i=window?.getComputedStyle(i).getPropertyValue(a),e=i||s);var a=`<svg width="${r}" height="${r}" xmlns="http://www.w3.org/2000/svg" viewBox="${t}" stroke="${e}" fill="${e}">
					<path d="${o}" stroke-width="2" />
				</svg>
			`,i=t&&o?`url('data:image/svg+xml;utf8,${encodeURIComponent(a)}')`:"25BC",s=l?.replace(/\./g,"");const c=n.querySelector(`#${s}-toc-style`);r=`
				${l} .${d}::before {
					content: ${i};
					display: inline-block;
					${t&&o?"transition: transform 0.3s ease":""};
					${t&&o?"transform: scale(0.7) translateX(-50%)":""};
				}
			`;c.textContent.includes(r)||(c.textContent+=r)}}},_setCollapseIconMargin(o,l){const s=UAGBTableOfContents._getDocumentElement(),n=s.querySelector(o),u=n.querySelector(".uagb-toc__loader");var r=n.querySelector("li.uagb-toc__list:not(.uagb-toc__list--expandable)");if(r){r=window.getComputedStyle(r).fontSize;const m=n.querySelector(".uagb-toc__list-wrap");var g=`calc(${r} / 3)`,a=o?.replace(/\./g,"");const c=s.querySelector(`#${a}-toc-style`),d=(c&&c.remove(),s.createElement("style")),p=(d.id=a+"-toc-style",navigator.userAgent.toLowerCase());var a=/^((?!chrome|android|crios|fxios).)*safari/i.test(p)&&!p.includes("edge"),i=p.includes("firefox");let e,t="-0.5px";"disc"===l?.markerView&&(a||i?(e=5+5*((parseFloat(r)-8)/8)+"px",t=i?"1px":"-0.5px"):e=r),"decimal"===l?.markerView&&(a=(e=>{e=parseFloat(e);return.25*e+2*Math.max(0,(e-16)/8)})(r),e=parseFloat(r)-a+"px",t="1px"),d.innerHTML+=`
				${o} .list-open::before,
