...
import NewInfoBoxPositionClasses from './style-classes';
import NewInfoBoxSeparator from './components/Separator';
import NewCallToAction from './components/CTA';
import NewIconImage from './components/IconImages';
...
const deprecated = [
	...
	},
	{
		...
		save( props ) {
			...
			if ( source_type === 'icon' && icon !== '' ) {
				isImage = <NewIcon attributes={ props.attributes } />;
			} else {
				isImage = <NewIconImage attributes={ props.attributes } />;
			}
		
			let iconImageHtml = isImage;
			let position = seperatorPosition;
			const seperatorHtml = <NewInfoBoxSeparator attributes={ props.attributes } />
			let showSeperator = true;
				...
				<>
					{ 'none' !== seperatorStyle &&
						...
						{ 'none' !== seperatorStyle &&
							...
							seperatorHtml }
						{ ctaType !== 'none' && (
							<NewCallToAction attributes={ props.attributes } />
						) }
				</>
			);
			...
			return (
				<div
					className={ classnames(
						...
						'uagb-infobox__content-wrap',
						ctaType === 'all' ? ' uagb-infobox_cta-type-all' : '',
						...NewInfoBoxPositionClasses( props.attributes )
					) }
				>
					{ ctaType === 'all' && (
						<a // eslint-disable-line jsx-a11y/anchor-has-content
