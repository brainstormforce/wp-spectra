/**
 * BLOCK: Info Box - Deprecated Save (v2.19.27 - v2.19.29)
 *
 * Save output used while the block was registered with apiVersion 3 but the
 * save function did not call useBlockProps.save(). For apiVersion 2+ blocks
 * Gutenberg only applies the `blocks.getSaveContent.extraProps` filters via
 * useBlockProps.save(), so blocks saved with these versions are missing the
 * generated `wp-block-uagb-info-box` class, the custom className, the anchor
 * id, and the `uag-hide-*` classes on the root element. This entry validates
 * that markup; `apiVersion: 3` on the deprecation entry keeps the legacy
 * extraProps injection disabled so the output matches byte-for-byte.
 */
