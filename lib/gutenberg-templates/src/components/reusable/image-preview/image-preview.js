...
const ImagePreview = ( {
	...
	const renderSelectionIcon = () => {
		...
		if ( variant === VARIANTS.selection ) {
			return (
				<button
					...
					className="flex items-center justify-center absolute top-2 right-2 p-1 bg-white rounded-full border border-solid border-zip-dark-theme-border cursor-pointer focus:outline-none focus-visible:outline-none"
				>
		...
		return (
			<div className="inline-flex absolute top-2 right-2 p-1 bg-image-selection rounded-full pointer-events-none">
			...
			<Tile
				...
				className={ classNames(
					...
					variant === VARIANTS.default &&
						isSelected && 'border-image-selection',
				) }
				onClick={ handleSelection( image ) }
			>
				<img
					className="inline-block h-fit w-full relative aspect-[12/8] bg-background-secondary"
