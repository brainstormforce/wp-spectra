...
const TagsInput = ( {
	...
	const filterDuplicateValues = ( val ) => {
		return filterDuplicates ? [ ...new Set( val ) ]?.filter( Boolean ) : val;
	};
	...
	const handlePaste = ( event ) => {
		...
		const newTags = pastedText
			...
			.filter( Boolean )
			.filter( ( tag ) => ! validationPattern || validationPattern.test( tag ) );
	...
	useEffect( () => {
		...
		if ( tags.length > 0 && tags.every( ( tag ) => tagValues.includes( tag ) ) ) {
			return;
		}
		setTags( filterDuplicateValues ? [ ...new Set( tagValues ) ] : tagValues );
	}, [ value ] ); // eslint-disable-line

	return (
		<div
			className={ classNames(
				'w-full min-h-[48px] flex items-center flex-wrap gap-2.5 rounded-md px-3 py-2.5 border border-border-primary border-solid ring-1 ring-transparent focus-within:ring-1 focus-within:ring-accent-spectra focus-within:outline-none placeholder:text-secondary-text text-app-text shadow-sm',
				className,
			) }
			onMouseDown={ ( event ) => {
