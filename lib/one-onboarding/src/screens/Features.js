...
const { screens, proSlug, selectAllProFeatures } =
	window.oneOnboardingData || {};
const screen = screens?.features || {};

/**
 * When enabled, toggling any single Pro feature checkbox selects or clears the
 * entire Pro set. Opt-in per product via the onboarding config
 * (`select_all_pro_features`) so the library stays product-agnostic — every
 * product that omits the flag keeps the default per-checkbox toggle.
 */
const useSelectAllProFeatures = !! selectAllProFeatures;

// All Pro feature IDs — used to select/clear the whole Pro set at once.
const proFeatureIds = ( screen?.featureList || [] )
...
const Features = () => {
	...
	// A feature checkbox is locked (always checked, non-interactive) when it is a
	// Free feature, or — only for products that opt into the Spectra Pro-as-a-set
	// behavior via `selectAllProFeatures` — a Pro feature while the Pro plugin is
	// already active. Other products keep their Pro checkboxes toggleable.
	const isFeatureLocked = ( feature ) =>
		! feature?.isPro ||
		( useSelectAllProFeatures && proStatus === 'active' );

	const handleFeatureToggle = ( feature, value ) => {
		// Locked features (Free, or an active-Pro feature under the Spectra
		// behavior) cannot be toggled off.
		...
		if ( useSelectAllProFeatures ) {
			// Spectra behavior: toggling any Pro feature selects or clears the
			// entire Pro set, so all Pro checkboxes move together.
			selectedFeatures = value ? [ ...proFeatureIds ] : [];
		} else {
	...
	const isFeatureChecked = ( feature ) => {
		...
		// Spectra behavior only: when the Pro plugin is active every Pro feature
		// is available, so it is shown checked and cannot be toggled off.
		if ( useSelectAllProFeatures && proStatus === 'active' ) {
			return true;
		}
		return proFeatures?.includes( feature.id );
	};
