...
const { screens, addonStatuses } = window.oneOnboardingData || {};
...
const AddOns = () => {
	// Seed from server-provided statuses so already installed/active plugins show
	// their status immediately; the async check below just refreshes it. Without
	// this the badge is blank until the REST round-trip resolves, and clicking
	// Continue in that window treats active plugins as not-installed.
	const [ addonStatus, setAddonStatus ] = useState( addonStatuses || {} );
