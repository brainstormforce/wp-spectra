...
class Sidebar_Configurations {
	...
	public function update_zip_ai_assistant_options( $params ) {
		...
		if ( ! empty( $last_message_tone ) ) {
			$current_options['last_used']['changeTone'] = [
				...
				'label' => __( ucfirst( $last_message_tone ) ), //phpcs:ignore WordPress.WP.I18n.NonSingularStringLiteralText
			];
		}
	...
	private function custom_message( $code ) {
		$message_array = array(
			'no_auth'              => __( 'Invalid auth token.' ),
			'insufficient_credits' => __( 'You have no credits left.' ),
		);
