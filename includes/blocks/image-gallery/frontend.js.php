...
$slick_options = apply_filters(
	...
	array(
		'arrows'        => is_bool( $attr['paginateUseArrows'] ) ? $attr['paginateUseArrows'] : (bool) $attr['paginateUseArrows'],
		'dots'          => is_bool( $attr['paginateUseDots'] ) ? $attr['paginateUseDots'] : (bool) $attr['paginateUseDots'],
		'initialSlide'  => is_int( $attr['carouselStartAt'] ) ? $attr['carouselStartAt'] : intval( $attr['carouselStartAt'] ),
		'infinite'      => is_bool( $attr['carouselLoop'] ) ? $attr['carouselLoop'] : (bool) $attr['carouselLoop'],
		'autoplay'      => is_bool( $attr['carouselAutoplay'] ) ? $attr['carouselAutoplay'] : (bool) $attr['carouselAutoplay'],
		'autoplaySpeed' => is_int( $attr['carouselAutoplaySpeed'] ) ? $attr['carouselAutoplaySpeed'] : intval( $attr['carouselAutoplaySpeed'] ),
		'pauseOnHover'  => is_bool( $attr['carouselPauseOnHover'] ) ? $attr['carouselPauseOnHover'] : (bool) $attr['carouselPauseOnHover'],
		'speed'         => is_int( $attr['carouselTransitionSpeed'] ) ? $attr['carouselTransitionSpeed'] : intval( $attr['carouselTransitionSpeed'] ),
		'slidesToShow'  => is_int( $attr['columnsDesk'] ) ? $attr['columnsDesk'] : intval( $attr['columnsDesk'] ),
		...
		'responsive'    => array(
			array(
				'breakpoint' => 1024,
				'settings'   => array(
					'slidesToShow' => intval( $attr['columnsTab'] ),
				),
			),
			array(
				'breakpoint' => 767,
				'settings'   => array(
					'slidesToShow' => intval( $attr['columnsMob'] ),
				),
			),
		),
	),
