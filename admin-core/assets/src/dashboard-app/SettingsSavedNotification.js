...
export default function SettingsSavedNotification() {
	...
	const renderIcon = () => {
		switch ( messageType ) {
			case 'warning':
				return <ExclamationCircleIcon className="h-4 w-4 text-amber-400" aria-hidden="true" />;
			case 'error':
				return <XCircleIcon className="h-4 w-4 text-red-500" aria-hidden="true" />;
			default:
				return '' === settingsSavedNotification ? (
					''
				) : (
						...
						<div className="max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden">
							<div className="p-4">
								<div className="flex items-center">
									<div  className="flex-shrink-0 flex items-center justify-center">{ renderIcon() }</div>
									<div className="ml-3 w-0 flex-1 text-sm font-medium text-gray-900">
										{ settingsSavedNotification }
									</div>
